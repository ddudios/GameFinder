...
final class PlatformDetailViewController: BaseViewController {

    // MARK: - Properties
    private let viewModel: PlatformDetailViewModel
    ...
    private func setupNavigationBar() {
        navigationItem.title = viewModel.platformName
    ...
    override func configureView() {
        super.configureView()
        // 플랫폼에 맞는 배경 이미지 설정
        backgroundImageView.image = UIImage(named: viewModel.platformName) ?? UIImage(named: "noImage")
    }
    ...
    // MARK: - Binding
    private func bind() {
        let input = PlatformDetailViewModel.Input(
    ...
    private func updateDataSource(with games: [Game]) {
        ...
        // 첫 번째 아이템: 헤더
        var items: [DetailItem] = [.header(title: viewModel.platformName, releaseDate: nil)]
...
extension PlatformDetailViewController: UICollectionViewDelegate {
